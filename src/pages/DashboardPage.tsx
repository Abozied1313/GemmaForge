...
import { useProjects, usePrompts, useTestRuns } from "@/hooks/useData";
...
const DashboardPage: React.FC = () => {
  ...
  const totalTokens = runs.reduce((acc, r) => acc + (r.tokensA ?? 0) + (r.tokensB ?? 0), 0);
  const avgTokens = runs.length ? Math.round(totalTokens / runs.length) : 0;

  // Model usage from A side (primary model)
  const modelUsage: Record<string, number> = {};
  runs.forEach((r) => {
    if (r.modelA) modelUsage[r.modelA] = (modelUsage[r.modelA] || 0) + 1;
    if (r.modelB) modelUsage[r.modelB] = (modelUsage[r.modelB] || 0) + 1;
  });
    ...
    <DashboardLayout>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-dark p-6 space-y-6">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(135deg, rgba(108,58,255,0.2) 0%, rgba(34,211,238,0.1) 100%)",
            ...
            <div className="space-y-2">
              {quickActions.map(({ icon: Icon, label, onClick, color }) => (
                <button
                  key={label}
                  onClick={onClick}
                  className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 hover:scale-[1.01]", isRTL && "flex-row-reverse")}
            ...
            {runs.length === 0 ? (
              ...
              <div className="space-y-2">
                {runs.slice(0, 6).map((run) => {
                  ...
                  const modelB = run.modelB ? GEMMA_MODELS.find((m) => m.id === run.modelB) : null;
                  return (
                    <div key={run.id}
                          ...
                          <p className="text-xs font-medium text-white truncate">
                            ...
                            {modelB && (
                              <span style={{ color: "#22d3ee" }}>
                                {" vs "}{lang === "ar" ? modelB.nameAr : modelB.nameEn}
                              </span>
                            )}
                          </p>
          ...
          <div className="rounded-xl p-5" style={{ background: "#0e1117", border: "1px solid #1e2535" }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {topModels.map(([modelId, count], i) => {
                ...
                const colors = ["#6c3aff", "#22d3ee", "#f59e0b"];
                return (
                  <div key={modelId} className="rounded-lg p-4" style={{ background: "#131720", border: "1px solid #1e2535" }}>
                    ...
                    <div className="w-full h-1.5 rounded-full" style={{ background: "#1e2535" }}>
                      <div className="h-full rounded-full"
                        style={{ width: `${(count / maxCount) * 100}%`, background: colors[i] }} />
                    </div>
